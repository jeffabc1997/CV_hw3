...
def main():
    ...
    #uniform_mean_shift_3d(monkey, 1000, 5, masterpiece_folder, "animal")
...
# def uniform_mean_shift_3d(path, bandwidth, converge_count, output_folder, output_name):
#     data = cv2.imread(path)
#     plot_rgb_original(data, output_folder, output_name)
#     height, width = data.shape[:2]
#     dataVector = np.ndarray(shape=(height* width , 3), dtype=np.float32)
#     dataVector_len = height*width
#     output_datavector = np.zeros(shape = dataVector.shape, dtype=np.float32)
#     for x in range(0, height):
#         for y in range(0, width):     
#             rgb = getpixel(data, x, y)
#             dataVector[y + x * width, 0] = rgb[0]
#             dataVector[y + x * width, 1] = rgb[1]
#             dataVector[y + x * width, 2] = rgb[2]
    
#     #print(dataVector[12])
#     for i in range(0, dataVector_len, 2):
#         cluster_centroid = dataVector[i] # point that we focused on
#         # Search points in circle
#         for count in range(converge_count):
...
#         output_datavector[i:i+2] = cluster_centroid
...
#     cv2.imwrite(f"./output/{output_folder}/2D_3d{output_name}_it{iteration_str}_band{bandwidth_str}.jpg", output)
# def uniform_mean_shift_5d(path, bandwidth, converge_count, output_folder, output_name):
#     data = cv2.imread(path) / 255 # normalize to (0, 1)
...
#             dataVector[y + x * width, 1] = rgb[1]
#             dataVector[y + x * width, 2] = rgb[2]
#             dataVector[y + x * width, 3] = x / height
#             dataVector[y + x * width, 4] = y / width
...
#             # Update centroid     
#             old_centroid = cluster_centroid
#             #new_centroid = np.round(np.average(temp_data, axis=0)) # mean of the points in the circle
#             cluster_centroid = new_centroid # shift to the mean
#             # print("old point ",old_centroid)
#             # Find the mode        
#             if np.array_equal(new_centroid, old_centroid):             
#                 break
...
#             print("order ",i)
#     #print("output datavector: ",output_datavector.shape)
#     output_datavector255 = output_datavector * 255
#     output = output_datavector255[:,:3].reshape(data.shape).astype(np.float32)
#     bandwidth_str = str(bandwidth)
#     iteration_str = str(converge_count)
#     plot_rgb_distribution(output_datavector, iteration_str, bandwidth_str, output_folder, output_name)
#     #plot_rgb_distribution(data, iteration, bandwidth, output_folder, output_name):
#     cv2.imwrite(f"./output/{output_folder}/2D_{output_name}_it{iteration_str}_band{bandwidth_str}.jpg", output)
# def uniform_mean_shift_5d(path, bandwidth):
#     data = cv2.imread(path) / 255 # normalize to (0, 1)
...
#         output_datavector[i] = cluster_centroid
#         if (i%20000) == 0:
#             print("order ",i)
#     #print("output datavector: ",output_datavector.shape)
#     output_datavector = output_datavector * 255
#     output = output_datavector[:,:3].reshape(data.shape).astype(np.float32)
#     band_str = str(bandwidth)
#     cv2.imwrite(f"./output/testMeanShiftNorm0_5.jpeg", output)
# def uniform_mean_shift_5d(path, bandwidth):
#     data = cv2.imread(path) 
...
#     output = output_datavector[:,:3].reshape(data.shape).astype(np.float32)
#     band_str = str(bandwidth)
#     cv2.imwrite(f"./output/testMeanShiftMatrix{band_str}.jpeg", output)
